...
class RateLimiter:
    ...
    def is_rate_limited(self, ip: str) -> bool:
        ...
        # Clean old entries
        self.requests_by_ip[ip] = [ts for ts in self.requests_by_ip[ip] if ts > window_start]
...
rate_limiter = RateLimiter(
    requests=config.RATE_LIMIT_REQUESTS, window_seconds=config.RATE_LIMIT_WINDOW
)


def verify_hmac_signature(
    timestamp: str, signature: str, body: bytes, secret: str | None = None
) -> bool:
    ...
    try:
        ...
        expected_signature = hmac.new(
            secret_to_use.encode(), message.encode(), hashlib.sha256
        ).hexdigest()
...
def verify_timestamp_window(
    timestamp_str: str, window_seconds: int = config.TIMESTAMP_WINDOW
) -> bool:
...
async def validate_webhook_security(request: Request, body: bytes) -> dict:
    ...
    return {"client_ip": client_ip, "event_id": event_id, "timestamp": timestamp, "valid": True}
